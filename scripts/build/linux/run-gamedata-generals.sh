#!/bin/bash
# GeneralsX @build caiiiycuk 24/07/2026 Copy runtime libs into a local gamedata dir and launch base GeneralsX from it.
# Usage: ./scripts/build/linux/run-gamedata-generals.sh [game_dir] [-- extra game args]
# Default game_dir: gamedata/default_ru/Generals
...
GAME_DIR="${1:-${PROJECT_ROOT}/gamedata/default_ru/Generals}"
...
if [[ ! -f "${BINARY}" ]]; then
    ...
    echo "Build first: cmake --build build/linux64-deploy --target g_generals"
    exit 1
fi
...
echo "==> Launching GeneralsX from ${GAME_DIR}"
cd "${GAME_DIR}"
